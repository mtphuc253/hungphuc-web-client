...
export function ArchitectureProjectList({
  initialProjects,
  initialMeta,
}: Props) {
  const [projects, setProjects] = useState<IProject[]>(initialProjects || []);
  const [meta, setMeta] = useState<ApiMeta | null>(initialMeta || null);
                  ...
                  <TableCell className="font-medium">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-xs text-muted-foreground line-clamp-3 max-w-[56ch]">
                          {project.description}
                        </div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="hidden md:table-cell">{capitalizeWords(project.subcategory)}</TableCell>
                  <TableCell className="hidden lg:table-cell">
                    {new Date(project.created_at ?? project.start_date ?? Date.now()).toLocaleDateString("vi-VN")}
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
